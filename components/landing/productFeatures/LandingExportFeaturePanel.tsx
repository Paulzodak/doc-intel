...
import { landingExportFeatureTabs, landingExportPanelCopy } from "./config";
import { easeOut } from "./motionVariants";

export function LandingExportFeaturePanel() {
  const [active, setActive] = useState(0);
  const current = landingExportFeatureTabs[active];
    ...
    <div className="flex h-full min-h-0 flex-col px-3.5 py-4 sm:px-5 sm:py-5 md:px-6 md:py-6">
      <header className="shrink-0">
        ...
        <p className="mt-1 text-center text-[10.5px] leading-snug text-gray-500 dark:text-gray-400 sm:mt-1.5 sm:text-[12px]">
          {landingExportPanelCopy.subtitle}
        </p>
      </header>
      <div
      ...
      >
        {landingExportFeatureTabs.map((f, i) => {
          ...
          return (
            <button
              ...
              aria-controls={`export-feature-${f.id}`}
              id={`export-tab-${f.id}`}
              onClick={() => setActive(i)}
              className={clsx(
      ...
      <div className="mt-2 shrink-0 border-t border-gray-100 pt-2 dark:border-white/10 sm:mt-3 sm:pt-3">
        <AnimatePresence mode="wait" initial={false}>
          <motion.p
            key={current.id}
            id={`export-feature-${current.id}`}
            role="tabpanel"
            aria-labelledby={`export-tab-${current.id}`}
