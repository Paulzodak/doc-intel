...
import { landingShareFeatureTabs, landingSharePanelCopy } from "./config";
import { easeOut } from "./motionVariants";

export function LandingShareFeaturePanel() {
  const [active, setActive] = useState(0);
  const current = landingShareFeatureTabs[active];
    ...
    <div className="flex h-full min-h-0 flex-col px-3.5 py-4 sm:px-5 sm:py-5 md:px-6 md:py-6">
        ...
        {landingShareFeatureTabs.map((f, i) => {
          ...
          return (
            <button
              ...
              aria-controls={`share-feature-${f.id}`}
              id={`share-tab-${f.id}`}
              onClick={() => setActive(i)}
              className={clsx(
      ...
      <div className="mt-2 shrink-0 border-t border-gray-100 pt-2 dark:border-white/10 sm:mt-3 sm:pt-3">
        <AnimatePresence mode="wait" initial={false}>
          <motion.p
            key={current.id}
            id={`share-feature-${current.id}`}
            role="tabpanel"
            aria-labelledby={`share-tab-${current.id}`}
